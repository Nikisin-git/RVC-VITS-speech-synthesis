"""Window: Demucs vocal/instrumental separation."""
...
class VocalRemoverWindow(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        fmt_row, self._fmt_group, self._rb_wav, _ = make_format_radio(self)
    ...
    def _run(self) -> None:
        ...
        dlg = ProgressDialog("Идёт отделение вокала от инструментала...", show_log=True, parent=self)
        ...
        def _next() -> None:
            if not remaining:
                ...
                dlg.append_log("=== Завершено ===")
                dlg.finish_success()
                return
            path = remaining.pop(0)
            dlg.set_status(f"Обработка: {Path(path).name}")
            worker = ProcessWorker("vocal_remover", SCRIPTS_DIR / "run_demucs.py",
            ...
            def _done(code: int) -> None:
                if code == 0:
                    ...
                    dlg.finish_error(f"Demucs завершился с кодом {code}")
